...
ereefs(get_ereefs_profile(var_names = "temp", start_date = c(2017, 11, 10), end_date = c(2017, 11, 10), location_latlon = c(-17.837, 146.710), input_file = 6))
```
...
```{r}
X1 <- c(gd_66a, gd_54a, gd_44a, gd_35a, gd_27a, gd_21a, gd_15a, gd_11a, gd_7a, gd_3.7a, gd_1a, gd_0a)
z_grid <- c(-66, -54, -44, -35, -27, -21, -15, -11, -7, -3.7, -1, 0)
data <- c("Glider")
gd_p <- data.frame(z_grid, X1, data)
```


```{r}
new_profile <- rbind(gd_p, pd_p)
```


```{r}
ggplot(new_profile, aes(X1, z_grid, colour=data), ) + geom_point() + geom_path() + theme_bw() + labs(title="Glider/eReefs Comparison 2017-11-10 GBR (-17.837, 146.710)",x="Temperature", y = "Depth (m) relative to msl")
